...
# }
PYTHIA_PARAMS_FOR_CASCADE = {'B': 64,'D': 9216,'E': 128,'F': 128,'H': 72,'M': 2048,'P': 2048,'S': 36864}
...
class Cascade:
    def __init__(self, model, seq_len):
        ...
        if model == "pythia" and seq_len == "1K":
            print(f"Cascade __init__: Directly instantiating Transformer for {model}")
            self.transformer = Transformer(**PYTHIA_PARAMS_FOR_CASCADE)
        elif model == "Pythia_SC_SG":
            print(f"Cascade __init__: Directly instantiating Transformer for {model}")
            self.transformer = Transformer(**PYTHIA_SC_SG_PARAMS)
        elif model == "Pythia_SC_LG":
            print(f"Cascade __init__: Directly instantiating Transformer for {model}")
            self.transformer = Transformer(**PYTHIA_SC_LG_PARAMS)
        elif model == "Pythia_LC_SG":
            print(f"Cascade __init__: Directly instantiating Transformer for {model}")
            self.transformer = Transformer(**PYTHIA_LC_SG_PARAMS)
